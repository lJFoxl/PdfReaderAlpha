...
namespace PdfSharp_net8_.iTextSharp.text.pdf.parser
{
    ...
    /*
ѕростое средство визуализации дл€ извлечени€ текста.
Ёто средство визуализации отслеживает текущую позицию Y каждой строки.  ≈сли оно обнаруживает
, что позици€ y изменилась, оно вставл€ет разрыв строки в выходные данные.  ≈сли
PDF-файл отображает текст не сверху вниз, это приведет к тому, что текст не
будет соответствовать истинному представлению о том, как он отображаетс€ в PDF-файле.
Ётот рендерер также использует простую стратегию, основанную на показател€х шрифта, чтобы определить
, следует ли вставл€ть пробел в выходные данные.
     */
    public class SimpleTextExtractionStrategy : ITextExtractionStrategy
    {
        ...
        /** используетс€ дл€ хранени€ результирующей строки. */
        private StringBuilder result = new StringBuilder();

        /**
         * —оздает новый рендерер дл€ извлечени€ текста.
         */
