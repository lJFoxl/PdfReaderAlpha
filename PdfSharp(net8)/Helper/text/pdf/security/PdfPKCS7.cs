...
using PdfSharp_net8_.Helper.text.error_messages;
...
namespace PdfSharp_net8_.Helper.text.pdf.security {
    ...
    public class PdfPKCS7 {
        ...
        public PdfPKCS7(ICipherParameters privKey, ICollection<X509Certificate> certChain, 
            ...
            digestalgos = new Dictionary<DerObjectIdentifier, object>();
        ...
        public PdfPKCS7(byte[] contentsKey, byte[] certsKey) {
            ...
            digestAlgorithmOid = new DerObjectIdentifier("1.2.840.10040.4.3");
        ...
        public PdfPKCS7(byte[] contentsKey, PdfName filterSubtype) {
            ...
            digestAlgorithmOid = ((DerObjectIdentifier)((Asn1Sequence)signerInfo[2])[0]);
            next = 3;
        ...
        private DerObjectIdentifier digestAlgorithmOid;
        
        /** The object that will create the digest */
        private IDigest messageDigest;
        
        /** The digest algorithms */
        private Dictionary<DerObjectIdentifier, object> digestalgos;
         ...
         */
        virtual public DerObjectIdentifier DigestAlgorithmOid {
            get {
        ...
        virtual public string GetHashAlgorithm() {
            return DigestAlgorithms.GetDigest(digestAlgorithmOid.Id);
        }
        ...
        virtual public byte[] GetEncodedPKCS7(byte[] secondDigest, ITSAClient tsaClient, byte[] ocsp, ICollection<byte[]> crlBytes, CryptoStandard sigtype) {
            ...
            foreach (var dal in digestalgos.Keys) {
        ...
        private DerSet GetAuthenticatedAttributeSet(byte[] secondDigest, byte[] ocsp, ICollection<byte[]> crlBytes, CryptoStandard sigtype) {
            ...
            if (sigtype == CryptoStandard.CADES) {
                ...
                var sha256Oid = DigestAlgorithms.GetAllowedDigests(DigestAlgorithms.SHA256);
                ...
                if (!sha256Oid.Equals(digestAlgorithmOid)) {
                    AlgorithmIdentifier algoId = new (digestAlgorithmOid);
                    aaV2.Add(algoId);
                }
