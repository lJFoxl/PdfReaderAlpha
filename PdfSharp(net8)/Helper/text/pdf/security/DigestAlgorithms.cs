...
namespace PdfSharp_net8_.Helper.text.pdf.security {
    ...
    public static class DigestAlgorithms {
        ...
        /**
         * Возвращает идентификатор алгоритма дайджеста, который разрешен в формате PDF,
         * or null if it isn't allowed. 
         * @param name  the name of the digest algorithm
         * @return  an oid
         */
        public static DerObjectIdentifier GetAllowedDigests(string name) {
            string ret;
            allowedDigests.TryGetValue(name.ToUpperInvariant(), out ret);
            
            return new DerObjectIdentifier(ret); ;
        }
