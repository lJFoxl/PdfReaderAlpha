...
using PdfSharp_net8_.Helper.text.pdf.codec;

namespace PdfSharp_net8_.Helper.text.pdf {
    ...
    public class BarcodeDatamatrix {
        ...
        */
        ...
        private static int B256Encodation(byte[] text, int textOffset, int textLength, byte[] data, int dataOffset, int dataLength, int symbolIndex, int prevEnc, int origDataOffset) {
            ...
            if (prevEnc == DM_B256)
                textLength = 1;
            global::System.Array.Copy(text, textOffset, data, minRequiredDataIncrement + dataOffset, textLength);
        ...
        private static int C40OrTextEncodation(byte[] text, int textOffset, int textLength, byte[] data, int dataOffset, int dataLength, bool c40, int symbolIndex, int prevEnc, int origDataOffset) {
            ...
            bool addLatch = true, usingASCII = false;
        ...
        private static int GetEncodation(byte[] text, int textOffset, int textSize, byte[] data, int dataOffset, int dataSize, int options, bool sizeFixed) {
            ...
            if (options == 0) {
                ...
                for (int i = 0; i < 6; i++) {
                    global::System.Array.Copy(data, 0, dataDynamic[i], 0, data.Length);
                    switchMode[i][0] = i + 1;
                }
                ...
                for (int i = 1; i < textSize; i++) {
                    ...
                    for (int currEnc = 0; currEnc < 6; currEnc++) {
                        ...
                        for (int prevEnc = 0; prevEnc < 6; prevEnc++) {
                            global::System.Array.Copy(dataDynamic[prevEnc], 0, dataDynamicInner[prevEnc], 0, data.Length);
                            if (currEnc == 0)
                        ...
                        if (switchMode[currEnc][i] != 0)
                            global::System.Array.Copy(dataDynamicInner[switchMode[currEnc][i] - 1], 0, dataDynamic[currEnc], 0, data.Length);
                    }
                }
                ...
                global::System.Array.Copy(dataDynamic[bestDataDynamicResultIndex], 0, data, 0, data.Length);
                return e;
            }
            switch (options) {
                case DM_ASCII:
        ...
        */
        virtual public int Generate(string text) {
            byte[] t = global::System.Text.Encoding.GetEncoding(encoding).GetBytes(text);
