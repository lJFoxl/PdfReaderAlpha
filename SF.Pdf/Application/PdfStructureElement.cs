    ...
    */
    ...
    public class PdfStructureElement : PdfDictionary, IPdfStructureElement
    {
        ...
        */
        ...
        virtual public PdfName StructureType => structureType;
        ...
        */
        ...
        virtual public PdfDictionary Parent => GetParent(false);
        ...
        internal virtual void SetAnnotation(PdfAnnotation annot, PdfIndirectReference currentPage) {
            var kArray = GetAsArray(PdfName.K);
            if (kArray == null) {
                kArray = new PdfArray();
                var k = Get(PdfName.K);
                if (k != null) {
        ...
        */
        ...
        virtual public PdfIndirectReference Reference => this.reference;
        ...
        public virtual void WriteAttributes(IAccessibleElement element) {
            ...
            if (element.GetAccessibleAttributes() != null) {
                foreach (var key in element.GetAccessibleAttributes().Keys) {
                    if (key.Equals(PdfName.ID)) {
                        var attr = element.GetAccessibleAttribute(key);
                        Put(key, attr);
            ...
            if (chunk != null)
            {
                ...
                else
                {
                    ...
                    if (attr != null)
                    {
                        ...
                        {
                            var back = (Object[]) attr[Chunk.BACKGROUND];
                            var color = (BaseColor) back[0];
                        ...
                        var parent = (IPdfStructureElement) this.GetParent(true);
                        var obj = GetParentAttribute(parent, PdfName.COLOR);
                        if ((chunk.Font != null) && (chunk.Font.Color != null))
                        {
                            var c = chunk.Font.Color;
                            SetColorAttribute(c, obj, PdfName.COLOR);
                        }
                        var decorThickness = GetParentAttribute(parent, PdfName.TEXTDECORATIONTHICKNESS);
                        var decorColor = GetParentAttribute(parent, PdfName.TEXTDECORATIONCOLOR);
                        if (attr.ContainsKey(Chunk.UNDERLINE))
                        {
                            var unders = (Object[][]) attr[Chunk.UNDERLINE];
                            var arr = unders[unders.Length - 1];
                            var color = (BaseColor) arr[0];
                            var floats = (float[]) arr[1];
                            var thickness = floats[0];
                            // Setting thickness
                            if (decorThickness is PdfNumber)
                            {
                                var t = ((PdfNumber) decorThickness).FloatValue;
                        ...
                        {
                            var height = (float) attr[Chunk.LINEHEIGHT];
                            var parentLH = GetParentAttribute(parent, PdfName.LINEHEIGHT);
                            if (parentLH is PdfNumber)
                            {
                                var pLH = ((PdfNumber) parentLH).FloatValue;
        ...
        private void WriteAttributes(Image image) {
            if (image != null) {
                ...
                var rect = new PdfRectangle(image, ((Rectangle) image).Rotation);
                this.SetAttribute(PdfName.BBOX, rect);
                if (image.BackgroundColor != null) {
                    var color = image.BackgroundColor;
        ...
        private void WriteAttributes(PdfTemplate template) {
            if (template != null) {
                ...
                var rect = new PdfRectangle(template.BoundingBox);
                this.SetAttribute(PdfName.BBOX, rect);
            }
        }
        ...
        private void WriteAttributes(Paragraph paragraph) {
            if (paragraph != null) {
                ...
                if (obj is PdfNumber) {
                    var startIndent = ((PdfNumber) obj).FloatValue;
                ...
                if (obj is PdfNumber) {
                    var endIndent = ((PdfNumber) obj).FloatValue;
        ...
        private void WriteAttributes(List list) {
            if (list != null) {
                ...
                var obj = GetParentAttribute(parent, PdfName.STARTINDENT);
                if (obj is PdfNumber) {
                    var startIndent = ((PdfNumber) obj).FloatValue;
        ...
        private void WriteAttributes(ListItem listItem) {
            if (listItem != null) {
                var obj = parent.GetParentAttribute(parent, PdfName.STARTINDENT);
                if (obj is PdfNumber) {
                    var startIndent = ((PdfNumber) obj).FloatValue;
        ...
        {
            var colorArr = new float[] {newColor.R/255f, newColor.G/255f, newColor.B/255f};
            if ((oldColor != null) && (oldColor is PdfArray))
            {
                var oldC = (PdfArray) oldColor;
        ...
        private void SetTextAlignAttribute(int elementAlign)
        {
            ...
            var obj = GetParentAttribute(parent, PdfName.TEXTALIGN);
            if (obj is PdfName)
            {
                var textAlign = ((PdfName) obj);
