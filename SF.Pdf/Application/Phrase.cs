    ...
    public class Phrase : List<IElement>, ITextElementArray {
        ...
        virtual protected bool AddChunk(Chunk chunk) {
            ...
            if (Count > 0 && !chunk.HasAttributes()) {
                try {
                    ...
                    var previousRole = previous.Role;
                    var chunkRole = chunk.Role;
                    bool sameRole;
        ...
        */
    	    ...
    	    if (font.Family != Font.FontFamily.SYMBOL && font.Family != Font.FontFamily.ZAPFDINGBATS && font.BaseFont == null) {
                ...
                while ((index = SpecialSymbol.Index(str)) > -1) {
                    if (index > 0) {
                        ...
                        p.Add((IElement)new Chunk(firstPart, font));
                        str = str.Substring(index);
                    }
                    var symbol = new Font(Font.FontFamily.SYMBOL, font.Size, font.Style, font.Color);
                    var buf = new StringBuilder();
                    ...
                    p.Add((IElement)new Chunk(buf.ToString(), symbol));
                }
            }
            if (!string.IsNullOrEmpty(str)) {
        	    p.Add((IElement)new Chunk(str, font));
            }
    	    return p;
        }
        ...
        virtual public bool Trim() {
            ...
            while (this.Count > 0) {
                var lastChunk = this[this.Count - 1];
