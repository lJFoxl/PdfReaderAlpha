    ...
    */
    public class PdfNumberTree {
        ...
        private static void IterateItems(PdfDictionary dic, Dictionary<int, PdfObject> items) {
            ...
            if (nn != null) {
                for (var k = 0; k < nn.Size; ++k) {
                    var s = (PdfNumber)PdfReader.GetPdfObjectRelease(nn.GetPdfObject(k++));
                    items[s.IntValue] = nn.GetPdfObject(k);
                }
            }
            else if ((nn = (PdfArray)PdfReader.GetPdfObjectRelease(dic.Get(PdfName.KIDS))) != null) {
                for (var k = 0; k < nn.Size; ++k) {
                    var kid = (PdfDictionary)PdfReader.GetPdfObjectRelease(nn.GetPdfObject(k));
        ...
        public static Dictionary<int, PdfObject> ReadTree(PdfDictionary dic) {
            var items = new Dictionary<int, PdfObject>();
            if (dic != null)
