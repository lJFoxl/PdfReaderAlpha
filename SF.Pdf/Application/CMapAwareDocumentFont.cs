...
namespace SF.Pdf.Application {
    ...
    public class CMapAwareDocumentFont : DocumentFont 
    {
        ...
        private void ProcessToUnicode(){
            var toUni = PdfReader.GetPdfObjectRelease(fontDic.Get(PdfName.TOUNICODE));
            if (toUni is PRStream) {
                ...
                try {
                    var encodingName = fontDic.GetAsName(PdfName.ENCODING);
                    if(encodingName == null)
                        return;
                    var enc = PdfName.DecodeName(encodingName.ToString());
                    if(!enc.Equals("Identity-H"))
                        return;
                    var df = (PdfArray)PdfReader.GetPdfObjectRelease(fontDic.Get(PdfName.DESCENDANTFONTS));
                    var cidft = (PdfDictionary)PdfReader.GetPdfObjectRelease(df[0]);
                    var cidinfo = cidft.GetAsDict(PdfName.CIDSYSTEMINFO);
                    if(cidinfo == null)
                        return;
                    var ordering = cidinfo.GetAsString(PdfName.ORDERING);
                    if(ordering == null)
                        return;
        ...
        private void ProcessUni2Byte(){
            ...
            if (toUnicodeCmap != null) {
                var dm = toUnicodeCmap.CreateDirectMapping();
        ...
        private int ComputeAverageWidth(){
            var count = 0;
            var total = 0;
            for (var i = 0; i < base.widths.Length; i++){
                if (base.widths[i] != 0){
                    total += base.widths[i];
                    count++;
                }
        ...
        private string DecodeSingleCID(byte[] bytes, int offset, int len){
            if (toUnicodeCmap != null){
                ...
                var s = toUnicodeCmap.Lookup(bytes, offset, len);
                if (s != null)
                    return s;
        ...
        virtual public string Decode(byte[] cidbytes, int offset, int len){
            ...
            if (toUnicodeCmap == null && byteCid != null) {
                var seq = new CMapSequence(cidbytes, offset, len);
                var cid = byteCid.DecodeSequence(seq);
                foreach (var ca in cid) {
                    var c = cidUni.Lookup(ca);
                    if (c > 0)
        ...
        virtual public string DecodeSingleCharacter(byte[] cidbytes, int offset, int len) {
            ...
            if (toUnicodeCmap == null && byteCid != null) {
                var seq = new CMapSequence(cidbytes, offset, len);
                var ca = byteCid.DecodeSingle(seq);
                if (ca >= 0) {
                    var c = cidUni.Lookup(ca);
                    if (c > 0)
                ...
                var rslt = DecodeSingleCID(cidbytes, offset, 1);
                if (rslt == null && len > 1) {
