...
namespace SF.Pdf.Application {
    ...
    public class PRTokeniser {
        ...
        virtual public long FilePointer => file.FilePointer;

        virtual public void Close() {
            file.Close();
        }
    
        virtual public long Length => file.Length;

        virtual public int Read() {
            return file.Read();
        }
    
        virtual public RandomAccessFileOrArray SafeFile => new RandomAccessFileOrArray(file);

        virtual public RandomAccessFileOrArray File => file;
        ...
        virtual public TokType TokenType => type;

        virtual public string StringValue => stringValue;

        /**
         * Gets current reference number. If parsing was failed with NumberFormatException -1 will be return.
         */
        virtual public int Reference => reference;
		...
		virtual public long LongValue => long.Parse (stringValue);

        virtual public int IntValue => int.Parse(stringValue);
		...
		public static long[] CheckObjectStart (byte[] line) {
            try {
                var tk = new PRTokeniser(new RandomAccessFileOrArray(new RandomAccessSourceFactory().CreateSource(line)));
                var num = 0;
                var gen = 0;
