    ...
    public class RadioCheckField : BaseField {
        ...
        */    
        public virtual PdfAppearance GetAppearance(bool isRadio, bool on) {
            ...
            if (fsize == 0) {
                float bw = ufont.GetWidthPoint(PdfSharp_net8_.iTextSharp.text, 1);
                if (bw == 0)
                    fsize = 12;
                else
                    fsize = wt / bw;
            ...
            app.SetTextMatrix((box.Width - ufont.GetWidthPoint(PdfSharp_net8_.iTextSharp.text, fsize)) / 2, 
                (box.Height - ufont.GetAscentPoint(PdfSharp_net8_.iTextSharp.text, fsize)) / 2);
            app.ShowText(PdfSharp_net8_.iTextSharp.text);
            app.EndText();
            app.RestoreState();
            return app;
        }
        ...
        */    
        protected virtual PdfFormField GetField(bool isRadio) {
            ...
            if (PdfSharp_net8_.iTextSharp.text != null)
                field.MKNormalCaption = PdfSharp_net8_.iTextSharp.text;
            if (rotation != 0)
