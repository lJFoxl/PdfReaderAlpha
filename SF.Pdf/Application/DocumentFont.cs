...
namespace SF.Pdf.Application {
    public class DocumentFont : BaseFont {
        ...
        // code, [glyph, width]
        private Dictionary<int, int[]> metrics = new();
        private string fontName;
        private PRIndirectReference refFont;
        private PdfDictionary font;
        private double[] fontMatrix;
        private IntHashtable uni2byte = new();
        private IntHashtable byte2uni = new();
        ...
        if (font.Get(PdfName.ENCODING) == null
                && drEncoding != null) {
            foreach (var key in drEncoding.Keys) {
        ...
        private void Init() {
            ...
            if (PdfName.TYPE1.Equals(subType) || PdfName.TRUETYPE.Equals(subType))
                ...
                var encodingName = font.GetAsName(PdfName.ENCODING);
                if (encodingName != null){
                    var enc = PdfName.DecodeName(encodingName.ToString());
                    var ffontname = CJKFont.GetCompatibleFont(enc);
                    if (ffontname != null) {
        ...
        virtual public PdfDictionary FontDictionary => font;
        ...
        private void ProcessType0(PdfDictionary font) {
            ...
            if (dwo != null)
                dw = dwo.IntValue;
            var widths = ReadWidths((PdfArray)PdfReader.GetPdfObjectRelease(cidft.Get(PdfName.W)));
            var fontDesc = (PdfDictionary)PdfReader.GetPdfObjectRelease(cidft.Get(PdfName.FONTDESCRIPTOR));
            FillFontDesc(fontDesc);
                    ...
                    if (ob.ToString().Equals("begin")) {
                        ...
                        while (true) {
                            ...
                            if (nx.ToString().Equals("endbfchar"))
                                break;
                            var cid = DecodeString((PdfString)nx);
                            var uni = DecodeString((PdfString)ps.ReadPRObject());
                            if (uni.Length == 1) {
                                var cidc = (int)cid[0];
                                var unic = (int)uni[uni.Length - 1];
                                var w = dw;
                        ...
                        while (true) {
                            ...
                            if (nx.ToString().Equals("endbfrange"))
                                break;
                            var cid1 = DecodeString((PdfString)nx);
                            var cid2 = DecodeString((PdfString)ps.ReadPRObject());
                            var cid1c = (int)cid1[0];
                            var cid2c = (int)cid2[0];
        ...
        private void DoType1TT() {
            ...
            var enc = PdfReader.GetPdfObject(font.Get(PdfName.ENCODING));
            if (enc == null) {
                var baseFont = font.GetAsName(PdfName.BASEFONT);
                ...
                if (toUnicode != null) {
                    var rm = toUnicode.CreateReverseMapping();
            ...
            if (BuiltinFonts14.ContainsKey(fontName)) {
                var bf = BaseFont.CreateFont(fontName, WINANSI, false);
        ...
        private void FillWidths() {
            var newWidths = font.GetAsArray(PdfName.WIDTHS);
            var first = font.GetAsNumber(PdfName.FIRSTCHAR);
            var last = font.GetAsNumber(PdfName.LASTCHAR);
            if (first != null && last != null && newWidths != null) {
                var f = first.IntValue;
                var nSize = f + newWidths.Size;
                if (widths.Length < nSize) {
                    var tmp = new int[nSize];
                    global::System.Array.Copy(widths, 0, tmp, 0, f);
                    widths = tmp;
                }
                ...
                for (var k = 0; k < diffs.Size; ++k) {
                    ...
                    if (obj.IsNumber())
                        ...
                        if (c != null && c.Length > 0) {
                            ...
                            var unicode = toUnicode.Lookup(new byte[]{(byte) currentNumber}, 0, 1);
        ...
        private CMapToUnicode ProcessToUnicode() {
            ...
            var toUni = PdfReader.GetPdfObjectRelease(this.font.Get(PdfName.TOUNICODE));
            if (toUni is PRStream) {
                try {
                    var touni = PdfReader.GetStreamBytes((PRStream)toUni);
        ...
        private void FillFontDesc(PdfDictionary fontDesc) {
            ...
            var maxAscent = Math.Max(ury, Ascender);
            var minDescent = Math.Min(lly, Descender);
        ...
        */
        public override int GetWidth(int char1) {
            if(isType0) {
                if(hMetrics != null && cjkMirror != null && !cjkMirror.IsVertical()) {
                    var c = cjkMirror.GetCidCode(char1);
                    var v = hMetrics[c];
                    if(v > 0)
                        return v;
                    else
        ...
        public override double[] GetFontMatrix() {
            if (fontMatrix == null) {
                var array = font.GetAsArray(PdfName.FONTMATRIX);
        ...
        */
        ...
        bool IsSymbolic()
        {
            var fontDescriptor = font.GetAsDict(PdfName.FONTDESCRIPTOR);
