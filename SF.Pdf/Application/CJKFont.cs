...
namespace SF.Pdf.Application {
    ...
    internal class CJKFont : BaseFont {
        ...
        private static void LoadProperties() {
            ...
            lock (allFonts) {
                ...
                try {
                    ...
                    {
                        foreach (var font in registryNames["fonts"].Keys) {
        ...
        private static void LoadRegistry() {
            ...
            using (var isp = StreamUtil.GetResourceStream(RESOURCE_PATH_CMAP + "cjk_registry.properties")) {
                p.Load(isp);
            }
            foreach (var key in p.Keys) {
                var value = p[key];
                var sp = value.Split(cspace, StringSplitOptions.RemoveEmptyEntries);
        ...
        private void LoadCMaps() {
            try {
                ...
                var registry = (string)fontDesc["Registry"];
                uniMap = "";
                foreach (var name in registryNames[registry + "_Uni"].Keys) {
                    uniMap = name;
        ...
        public static bool IsCJKFont(string fontName, string enc) {
            ...
            if (enc.Equals(IDENTITY_H) || enc.Equals(IDENTITY_V))
                return true;
            var registry = allFonts[fontName]["Registry"] as string;
        ...
        private float GetBBox(int idx) {
            var s = (string)fontDesc["FontBBox"];
         ...
         */
        public override string[][] FamilyFontName => this.FullFontName;
        ...
        internal static Dictionary<string, object> ReadFontProperties(string name) {
            name += ".properties";
            var isp = StreamUtil.GetResourceStream(RESOURCE_PATH_CMAP + name);
            var p = new Properties();
            p.Load(isp);
            isp.Close();
